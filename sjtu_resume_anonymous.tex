...
\begin{tikzpicture}[remember picture,overlay]
  ...
  \node[anchor=east,inner sep=0pt] at ([xshift=14.92cm,yshift=-1.64cm]current page.north west){{\fontsize{26}{28}\selectfont\bfseries\color{sjtuBlue}克劳德}};
  % Contact details fill the gray area to the left of the photo in two aligned columns.
  \node[anchor=west,align=left,font=\fontsize{12}{18}\selectfont] at ([xshift=1.68cm,yshift=-3.55cm]current page.north west){\textbf{电话：}xxxxxxxxxxx\\[1pt]\textbf{邮箱：}xxxxxx@sjtu.edu.cn};
  \node[anchor=west,align=left,font=\fontsize{12}{18}\selectfont] at ([xshift=10.25cm,yshift=-3.55cm]current page.north west){\textbf{出生日期：}20xx.xx.xx\\[1pt]\textbf{政治面貌：}中共党员};
  ...
  \node[inner sep=0pt] at ([xshift=17.05cm,yshift=-2.65cm]current page.north west){\includegraphics[width=3.08cm,height=3.38cm,keepaspectratio]{claude_shannon.png}};
\end{tikzpicture}
\vspace*{3.38cm}
